...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Get user from registrations table
  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      ...
      return res.status(200).json({
        id: updateResult.rows[0].hotel_id,
        name: updateResult.rows[0].shop_name || updateResult.rows[0].owner_name,
        phone: updateResult.rows[0].phone_number,
        role: role || 'user'
      });
    }
