...
// Get items for a specific registration
...
// Add a new item for a registration
app.post('/registrations/:hotelId/items', async (req, res) => {
  const { hotelId } = req.params;
  const { item_id, name, price, unit, quantity } = req.body;

  try {
    const result = await pool.query(
      'INSERT INTO registration_items (registration_id, item_id, name, price, unit, quantity) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [hotelId, item_id, name, price, unit, quantity]
    );
...
// Update an item for a registration
...
// Delete an item for a registration
app.delete('/registrations/items/:itemId', async (req, res) => {
  const { itemId } = req.params;

  try {
    const result = await pool.query('DELETE FROM registration_items WHERE id = $1 RETURNING *', [itemId]);
    if (result.rowCount === 0) {
      return res.status(404).json({ success: false, error: 'Item not found' });
    }
